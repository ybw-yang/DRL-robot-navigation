...
namespace gazebo_ros
{
  ...
  public:
    ...
    bool setModelState(
        gazebo_msgs::srv::SetModelState::Request::SharedPtr req,
        gazebo_msgs::srv::SetModelState::Response::SharedPtr res);
    ...
    rclcpp::Service<gazebo_msgs::srv::SetModelState>::SharedPtr set_model_state_service_;
  ...
  void GazeboRosFactoryPrivate::OnWorldCreated(const std::string &_world_name)
  {
    ...
    set_model_state_service_ = ros_node_->create_service<gazebo_msgs::srv::SetModelState>(
        "gazebo/set_model_state",
        std::bind(
            &GazeboRosFactoryPrivate::setModelState, this,
            std::placeholders::_1, std::placeholders::_2));
  ...
  bool GazeboRosFactoryPrivate::setModelState(
      gazebo_msgs::srv::SetModelState::Request::SharedPtr req,
      gazebo_msgs::srv::SetModelState::Response::SharedPtr res)
  {
    ...
    if (!model)
    {
      RCLCPP_ERROR(ros_node_->get_logger(), "Updating ModelState: model [%s] does not exist", req->model_state.model_name.c_str());
      res->success = false;
    ...
    else
    {
      ...
      {
        RCLCPP_DEBUG(ros_node_->get_logger(), "Updating ModelState: reference frame is empty/world/map, usig inertial frame");
      }
      else
      {
        RCLCPP_ERROR(ros_node_->get_logger(), "Updating ModelState: for model[%s], specified reference frame entity [%s] does not exist",

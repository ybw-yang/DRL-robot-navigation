...
class td3(object):
    ...
    def train(
        ...
        env.get_logger().info(f"loss, Av.Q, Max.Q, target_Q, iterations : {av_loss / self.iter_count}, {av_Q / self.iter_count}, {max_Q}, {torch.max(target_Q)}, {self.iter_count}")
        self.writer.add_scalar("loss", av_loss / self.iter_count, self.iter_count)
        self.writer.add_scalar("Av. Q", av_Q / self.iter_count, self.iter_count)
        self.writer.add_scalar("Max. Q", max_Q, self.iter_count)
        self.writer.add_scalar("target. Q", torch.max(target_Q), self.iter_count)
...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.set_self_state.model_name = "td_robot"
...
if __name__ == '__main__':
    ...
    env = GazeboEnv()
    env.get_logger().info("------------------------------------Using device: {}".format(device))
